...
internal fun StopWatch(
    ...
    Row(
        modifier = modifier.testTag(tag = StopWatch.Tag.root),
        horizontalArrangement = stowWatchHorizontalArrangement,
        verticalAlignment = Alignment.CenterVertically
    ) {
        Icon(
            modifier = Modifier
                .testTag(tag = StopWatch.Tag.icon)
                        ...
                        AnimatedContent(
                            ...
                            StopWatchChar(
                                modifier = Modifier.testTag(
                                    tag = StopWatch.Tag.timerChar(index)
                                ),
                                char = digit
                            )
                        }
...
private fun Long.formatTime(): String {
    ...
    return String.format(stowWatchTimeFormat, minutes, seconds, millis)
}

object StopWatch {
    object Tag {
        const val root = "StopWatch"
        const val icon = "$root.Icon"
        fun timerChar(index: Int) = "$root.TimerChar.$index"
    }
}
